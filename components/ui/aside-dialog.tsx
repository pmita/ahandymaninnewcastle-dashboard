...
export const DialogPortal = React.forwardRef<HTMLDivElement, IDialog>(({ className, children, ...props }) => {
  return (
    ReactDOM.createPortal((
      <div className={cn(
        "bg-slate-900/20 backdrop-blur fixed inset-0 z-50 flex flex-row justify-end items-center overflow-y-scroll cursor-pointer",
        className
      )}
      {...props}
      >
...
export const DialogOverlay = React.forwardRef<HTMLDivElement, IDialog>(({ className, children, ...props }, ref) => {
  return (
    <div className={cn(
      "bg-gradient-to-br text-white p-6 rounded-lg w-full max-w-lg shadow-xl cursor-default relative overflow-hidden",
      className
    )}
    {...props}
    ref={ref}
...
export const DialogContent = React.forwardRef<HTMLDivElement, IDialog>(({ className, children, ...props }, ref) => {
  return (
    <DialogPortal>
      <DialogOverlay />
      <div className={cn(
        "bg-primary fixed z-50 grid h-full w-full rounded-b-lg border shadow-lg animate-in sm:max-w-lg sm:rounded-lg sm:zoom-in-90",
        className
        )}
        {...props}
        ref={ref}
